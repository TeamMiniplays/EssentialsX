...
public class Commandbalance extends EssentialsCommand {
    ...
    protected void run(final Server server, final CommandSource sender, final String commandLabel, final String[] args) throws Exception {
        ...
        sender.sendTl("balanceOther", target.isHidden() ? target.getName() : target.getName(), NumberUtil.displayCurrency(target.getMoney(), ess));
    }
    ...
    public void run(final Server server, final User user, final String commandLabel, final String[] args) throws Exception {
        if (args.length == 1 && user.isAuthorized("essentials.balance.others")) {
            ...
            user.sendTl("balanceOther", target.isHidden() ? target.getName() : target.getName(), NumberUtil.displayCurrency(target.getMoney(), ess));
